...
const funcFilter = (input) => {
    let arrOutput=[]

    input.data.response.billdetails.map((val)=>{
        let splitDenom = val.body[0].split(' ')
        let target = parseInt(splitDenom[2]) 
        if(target >= 100000){
            arrOutput.push(target)
        }
    })
    return arrOutput
}

console.log(funcFilter(data))